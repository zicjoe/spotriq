...
function required(v:string|undefined,label:string,max=2048){try{return normalizeUntrustedText(v??"",label,max,{required:true});}catch(error){throw new AgentStudioError(error instanceof Error?error.message:`${label} is invalid.`,"INVALID_INPUT");}}
function https(v:string,label:string){try{return validateExternalHttpUrl(required(v,label),{label}).toString();}catch(error){throw new AgentStudioError(error instanceof Error?error.message:`${label} must be a safe external HTTPS URL.`,"INVALID_INPUT");}}
