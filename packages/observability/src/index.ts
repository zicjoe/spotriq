...
async function chainComponent(chain: BscChainReader, now: Date): Promise<OperationalHealthComponent> {
  ...
  try {
    ...
    const state: OperationalHealthComponent["state"] = ok === 0 ? "UNAVAILABLE" : mismatch > 0 || unavailable > 0 ? "DEGRADED" : "OK";
    return component({
      ...
      summary: state === "OK" ? `${ok} BSC RPC endpoint${ok===1?"":"s"} responded on expected chain ${status.expectedChainId}.` : state === "DEGRADED" ? "At least one BSC RPC endpoint is degraded while another remains usable." : "No configured/fallback BSC RPC endpoint is currently usable on the expected chain.",
      metrics:{ expectedChainId:status.expectedChainId, endpointCount:status.endpoints.length, healthyEndpoints:ok, unavailableEndpoints:unavailable, chainMismatchEndpoints:mismatch, latestBlockNumber:status.latestBlockNumber??null, rpcMode:status.rpcMode },
      diagnostics:status.endpoints.map(item=>`${item.role} ${safeEndpointRef(item.url)}: ${item.state}${item.latencyMs!==undefined?` (${item.latencyMs}ms)`:""}${item.detail?` — ${sanitizeDiagnostic(item.detail)}`:""}`),
      limitations:["RPC health confirms endpoint reachability and chain identity; it does not establish protocol correctness, financial readiness, or transaction finality for unrelated operations."],
    });
  } catch (error) {
