...
function createHttp402Adapter(rail:"X402"|"B402",chain:BscChainReader):CommercialPaymentAdapter{
  return {rail,async reconcile(ctx){
    ...
    if(receipt.transactionHash.toLowerCase()!==hash||tx.hash.toLowerCase()!==hash)throw new CommercialError(`${rail} chain reader returned transaction evidence for a different hash.`,"ONCHAIN_OBSERVATION_FAILED",true);
    const blockCoherent=(!tx.blockNumber||tx.blockNumber===receipt.blockNumber)&&(!tx.blockHash||tx.blockHash.toLowerCase()===receipt.blockHash.toLowerCase());
    if(!blockCoherent)throw new CommercialError(`${rail} transaction and receipt block evidence do not reconcile.`,"ONCHAIN_OBSERVATION_FAILED",true);
    const block=await chain.getBlock(receipt.blockNumber);
    const transfer=(receipt.logs??[]).find(log=>log.logIndex!==undefined&&log.address===token&&log.topics[0]?.toLowerCase()===TRANSFER_TOPIC&&log.topics.length>=3&&topicAddress(log.topics[1])===ctx.hire.buyerAddress&&topicAddress(log.topics[2])===payTo&&transferValue(log.data)===expected);
    const success=receipt.status==="SUCCESS";
    const blockTime=Date.parse(block.timestamp), acceptedAt=Date.parse(ctx.hire.acceptedAt), futureLimit=ctx.now.getTime()+300_000;
    const timingOk=Number.isFinite(blockTime)&&Number.isFinite(acceptedAt)&&blockTime>=acceptedAt&&blockTime<=futureLimit;
