...
import type { AltanaGrantProof, BoundedPermissionRequest, RebalancingJobIntent } from "@spotriq/domain";
...
test("prepare derives exact V3 contract/signature scope and user-proposed token caps", async () => {
  ...
  assert.deepEqual(request.safetyPrerequisites.map((item) => item.code), ["TRUSTED_AGENT_SESSION_KEY", "ARGUMENT_LEVEL_EXECUTION_GUARD"]);
...
test("prepare rejects unconfirmed jobs and unmodeled Infinity CL authority", async () => {
  ...
  await assert.rejects(() => engine.prepare(infinity, { token0Limit: "1", token1Limit: "1", validForMinutes: 30 }, now), /only derives exact selector-scoped authority for PancakeSwap V3/);
});
...
test("exact provider scope plus current Keystore validity becomes ACTIVE but still non-executable", async () => {
  ...
  assert.deepEqual(grant.executionSafetyPrerequisites.map((item) => item.code), ["TRUSTED_AGENT_SESSION_KEY", "ARGUMENT_LEVEL_EXECUTION_GUARD"]);
