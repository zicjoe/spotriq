...
export function createPermissionCheckoutEngine(options:{store?:PermissionCheckoutStore;commercial:CommercialEngine;marketplace:MarketplaceSupplyReader;jobs?:PermissionCheckoutJobReader;authority?:PermissionCheckoutAuthorityBridge;now?:()=>Date}):PermissionCheckoutEngine{
  ...
  async function create(activationId:string,input:CreatePermissionCheckoutInput):Promise<PermissionCheckout>{
    ...
    } else if(category==="grid") blockers.push(blocker("GRID_EXECUTION_ADAPTER_REQUIRED","Grid execution adapter required","GridPilot currently provides deterministic market-context observation only. A bounded trading execution adapter and argument-level guard must exist before write authority can be submitted."));
    else if(category==="yield") blockers.push(blocker("YIELD_EXECUTION_ADAPTER_REQUIRED","Yield execution adapter required","YieldPilot currently observes supported yield opportunities only. Supply/withdraw execution scope must be modeled and guarded before write authority can be submitted."));
    else blockers.push(blocker("HEALTH_PROTECTIVE_WRITE_ADAPTER_REQUIRED","Protective-write adapter required","VenusGuard currently monitors health only. Repay/add-collateral intervention must have an independently modeled protective-write adapter before authority can be submitted."));
    const provider=category==="rebalancing"?"ALTANA" as const:"UNASSIGNED" as const; const providerSubmissionState=blockers.length?category==="rebalancing"&&blockers.every(b=>b.code==="REBALANCING_JOB_INTENT_REQUIRED")?"JOB_INTENT_REQUIRED" as const:category!=="rebalancing"?"CATEGORY_ADAPTER_REQUIRED" as const:"BLOCKED" as const:"READY_FOR_PROVIDER" as const;
    const checkoutId=id("permission-checkout",buyer,key),createdAt=at.toISOString(),scopeHash=requestedScopeHash; const checkout:PermissionCheckout={checkoutId,activationId:activation.activationId,serviceId:activation.serviceId,buyerAddress:buyer,category,state:blockers.length?"BLOCKED":"READY_FOR_REVIEW",idempotencyKey:key,scope,scopeHash,commercialTermsHash:activation.termsHash,permissionProfileSnapshot:record.permissionProfile,cost:costs(activation),risk:risks(category),blockers,provider,providerSubmissionState,linkedJobIntentId:linkedJob?.jobIntentId??input.linkedJobIntentId,reviewSummary:review(scope,record.service.name),createdAt,updatedAt:createdAt,expiresAt:scope.expiresAt,methodVersion:PERMISSION_CHECKOUT_METHOD,limitations:["PermissionProfile ≠ PermissionCheckout ≠ ScopedPermissionRequest ≠ PermissionGrant.","Payment, commercial Activation and financial Permission remain independent resources.","Current read-only reference services remain blocked from write authority until their service declaration, financial readiness and category execution safeguards are real."]}; await store.saveCheckout(checkout); return checkout;
  }
