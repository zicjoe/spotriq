...
function canonicalPlanHash(plan: Omit<RebalancingExecutionPlan, "planHash" | "steps">, steps: Array<Pick<RebalancingExecutionPlanStep,"index"|"kind"|"callHash">>): string {
  return hash(JSON.stringify({ jobIntentId: plan.jobIntentId, permissionRequestId: plan.permissionRequestId, serviceId: plan.serviceId, walletAddress: plan.walletAddress.toLowerCase(), targetRange: plan.targetRange, quote: { quoteId: plan.quote.quoteId, blockNumber: plan.quote.blockNumber, expectedDecreaseAmount0Raw: plan.quote.expectedDecreaseAmount0Raw, expectedDecreaseAmount1Raw: plan.quote.expectedDecreaseAmount1Raw, expectedCollectAmount0Raw: plan.quote.expectedCollectAmount0Raw, expectedCollectAmount1Raw: plan.quote.expectedCollectAmount1Raw }, steps }));
}
...
export function createExecutionPlanEngine(options: { store?: ExecutionPlanStore; pancakeSwap: PancakeSwapReader }): ExecutionPlanEngine {
  ...
  async function prepareInternal(job: RebalancingJobIntent, request: BoundedPermissionRequest, input: PrepareExecutionPlanInput, now: Date, reviewed: boolean): Promise<RebalancingExecutionPlan> {
    ...
    const targetRange = { tickLower: input.targetTickLower, tickUpper: input.targetTickUpper, tickSpacing: position.pool.tickSpacing, currentTickAtReview: position.pool.currentTick, state: reviewed ? "USER_REVIEWED" as const : "PROPOSED" as const, proposedBy: "USER" as const, reviewedAt: reviewed ? createdAt : undefined, detail: reviewed ? "The user explicitly confirmed this replacement range after Spotriq refreshed the V3 position and quote." : "User-proposed replacement range awaiting explicit execution-plan review." };
    ...
    const base = {
      ...
      limitations: ["The execution plan is deterministic from one reviewed Job Intent, one live V3 position snapshot, one independent eth_call simulation, and one user-reviewed replacement range.", "No transaction is signed or submitted by this plan.", "The external AgentService is an authenticated proposer only. It will not receive the future financial signing key.", "A fresh boundary preflight must revalidate LP ownership/state and output floors before any future signing.", ...(job.constraints.allowSwapPreparation ? ["Swap preparation was allowed by the Job Intent, but v0.17 intentionally creates no router swap step. The plan only reuses the tokens expected from the existing position."] : [])],
    };
